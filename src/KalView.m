...
- (void)WACal_addSubviewsToHeaderView:(UIView *)headerView;
- (void)WACal_addSubviewsToContentView:(UIView *)contentView;
...
- (id)initWithFrame:(CGRect)frame delegate:(id<KalViewDelegate>)theDelegate logic:(KalLogic *)theLogic WACalStyle:(BOOL)WACalStyle
{
  ...
  if ((self = [super initWithFrame:frame])) {
    ...
    if (WACalStyle)
      [self WACal_addSubviewsToHeaderView:headerView];
    else
    ...
    if (WACalStyle)
      [self WACal_addSubviewsToContentView:contentView];
    else
...
{
  return [self initWithFrame:frame delegate:theDelegate logic:theLogic WACalStyle:NO];
}
...
- (void)WACal_addSubviewsToContentView:(UIView *)contentView
{
  ...
  gridView = [[KalGridView alloc] initWithFrame:fullWidthAutomaticLayoutFrame logic:logic delegate:delegate WACalStyle:YES];
...
{
  [gridView markTilesForDates:dates WACalStyle:WACalStyle];
}
